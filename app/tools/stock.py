...
from app.dependencies import exa, logger
...
def normalize_stock_candidates(raw_query: str) -> list[str]:
    ...
    primary_symbol = alias_map.get(raw_query.lower(), raw_query.upper())
    symbols = [primary_symbol]
    if raw_query.isdigit() and len(raw_query) == 6:
        symbols.extend([f"{raw_query}.KS", f"{raw_query}.KQ", raw_query])
